...
export const MUSIC_LESSONS: MusicStudy[] = [
  lesson(
    'balanced-touch',
    tr('A balanced seat and an easy touch'),
    0,
    tr(
    ...
    [
      tr('My shoulders remain easy while I play.'),
      tr('I can release each key without holding the rest of my hand rigid.'),
    ],
    [
      note(60, 0, 0.7, 'rh', 1),
      note(62, 2, 0.7, 'rh', 2),
  ...
  lesson(
    'pulse-subdivision',
    tr('One beat, then two equal parts'),
    0,
    tr(
    ...
    [tr('I count the same pulse through both note speeds.')],
    [
  ...
  lesson(
    ...
    [
      tr('I release for the silent beats.'),
      tr('I hold the long note through the next beat without pressing again.'),
    ],
    [0, 4, 8, 12].flatMap((b) => [note(60, b, 0.7), note(60, b + 2, 1.9)]),
  ),
  lesson(
    'alternating-hands',
    tr('Two hands taking turns'),
    0,
    tr(
    ...
    [tr('I can keep counting when the hands change.'), tr('Both hands begin together in the second half.')],
    [
  ...
  lesson(
    'connected-detached',
    tr('Connected notes and light endings'),
    1,
    tr(
    ...
    [
      tr('The connected version has no unintended gaps.'),
      tr('The detached version has clear silence without a forced wrist movement.'),
    ],
    [
  ...
  lesson(
    ...
    [
      tr('My hand follows the thumb instead of twisting.'),
      tr('The notes before and after the thumb change sound even.'),
    ],
  ...
  lesson(
    'sustained-bass',
    tr('A moving melody above a held bass'),
    1,
    tr(
    ...
    [tr('The bass stays down while the melody changes.'), tr('I can hear the melody above the bass.')],
    [0, 4, 8, 12].flatMap((b) => [
      note(48, b, 3.9, 'lh', 5),
  ...
  lesson(
    ...
    [tr('I count three beats in every bar.'), tr('The first beat feels clear without a hard accent.')],
    [0, 3, 6, 9].flatMap((b) => [note(48, b, 2.8, 'lh'), note(64, b + 1), note(67, b + 2)]),
    { beatsPerBar: 3 },
  ),
  lesson(
    'offbeats',
    tr('Keep the pulse through offbeats'),
    2,
    tr(
    ...
    [
      tr('I can say the numbers while playing only on and.'),
      tr('The two hands alternate at equal distances.'),
    ],
    Array.from({ length: 16 }, (_, i) => [note(48, i, 0.4, 'lh'), note(60, i + 0.5, 0.4)]).flat(),
  ),
  lesson(
    'rhythm-echo',
    tr('Listen and repeat a rhythm'),
    2,
    tr(
    ...
    [tr('I can clap the pattern before I play.'), tr('I keep counting through silence.')],
  ...
  lesson(
    ...
    [
      ...
      tr('My phrase has a direction and a relaxed ending.'),
    ],
    [0, 4, 8, 12].flatMap((b) => [
      note(48, b, 3.7, 'lh'),
  ...
  lesson(
    ...
    [
      tr('I can hear each new chord without the previous harmony blurring it.'),
      tr('I can play the phrase without pedal too.'),
    ],
    [0, 4, 8, 12].flatMap((b, i) => (i % 2 ? [53, 57, 60] : [48, 52, 55]).map((m) => note(m, b, 3.5, 'lh'))),
  ),
  lesson(
    'melody-echo',
    tr('Listen, sing, then play'),
    3,
    tr(
    ...
    [tr('I sing the whole phrase before playing.'), tr('I can repeat it after a short pause.')],
  ...
  lesson(
    ...
    tr('Recover a melody and its rhythm'),
    7,
    tr(
    ...
    [
      tr('My written or remembered version includes the silent spaces and long notes.'),
      tr('I can play the whole phrase from memory on another day.'),
    ],
    [
      note(60, 0),
      note(64, 1, 0.4),
...
function piece(
  ...
  return musicStudySchema.parse({
    ...
    selfChecks: [
      tr('I can play the final phrase without stopping.'),
      tr('I can hear the melody above the accompaniment.'),
      tr('I leave a small breath between phrases.'),
    ],
  });
}
